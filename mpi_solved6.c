 ...
 *   4 tasks are required. Fixed.
 * AUTHOER: Manyuan Tao
 * LAST REVISED: 04/12/17
 ******************************************************************************/

/*
 BUG1: rank 2 uses a blocking send(MPI_Send), which should not run into MPI_Waitall().
 SOLUTION: exclude rank 2 from those running into MPI_Waitall().
 BUG2: 'offset' for rank 1 should be initialized with 0 since it will exceed the size of reqs array if initialized with REPS.
 SOLUTION: initialize 'offset = 0' for rank 1.
*/
...
int main (int argc, char *argv[])
{
    ...
    if (rank < 2) {
        ...
        if (rank == 1) {
            src = 0;
            offset = 0;   //!BUG2: initialize 'offset = 0' for rank 1
        }
        dest = src;
    ...
    if (rank != 2)   //!BUG1: rank 2 should be excluded from waiting
